...
add('Workbench navigation covers all ten calculators',['ascvd','diabetes','vancomycin','renal','opioid','anion','calcium','phenytoin','heparin','bmi'].every(k=>source.includes(`id:'${k}'`)||demo.includes(`data-tool="${k}"`))&&source.includes('CLINICAL_CALCULATOR_GROUPS'));
add('Workbench renders one active panel with responsive navigation',source.includes('setupClinicalCalculatorWorkbench')&&source.includes('panel.hidden=!active')&&source.includes('clinicalCalculatorSelect')&&demo.includes("setupClinicalCalculatorWorkbench(ov,focus||'ascvd')")&&demo.includes('.calc-workbench-shell')&&demo.includes('@media(max-width:1024px)'));
