...
(()=>{
 ...
 const mcq=cases.filter(x=>x.type==='mcq'),positions={},optionCounts={},shortFeedbackItems=[];let malformed=0,shortFeedback=0;
 mcq.forEach(x=>{const os=x.c.opts||[];optionCounts[os.length]=(optionCounts[os.length]||0)+1;const ok=os.map((o,j)=>o.ok?j:-1).filter(j=>j>=0);if(ok.length!==1)malformed++;else positions[ok[0]+1]=(positions[ok[0]+1]||0)+1;os.forEach((o,j)=>{if(String(o.fb||'').trim().length<25){shortFeedback++;shortFeedbackItems.push({module:x.module,index:x.index,option:j,text:String(o.fb||''),question:x.rawQ})}})});
 const generic=[
