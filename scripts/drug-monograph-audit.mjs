...
add('Tools exposes a searchable 100-drug reference',demo.includes('function showDrugReference')&&demo.includes('function drugReferenceKeys')&&demo.includes('placeholder="Search medication, class, or use"')&&demo.includes('<b>Drug Reference</b>'));
add('Drug Reference uses a top search with full-width monograph instead of a fixed sidebar',demo.includes('grid-template-rows:auto auto minmax(0,1fr)')&&demo.includes('<div class="drug-ref-search-wrap"><div class="drug-ref-search-shell">')&&!demo.includes('class="drug-ref-index"'));
add('Drug Reference supports compact search results and A-Z browsing',demo.includes('function drugReferenceAZHTML')&&demo.includes('function browseDrugReferenceLetter')&&demo.includes('class="drug-ref-list" id="drugRefList" aria-label="Medication search results"'));
add('Drug Reference hides global utilities while its dialog is open',demo.includes('body:has(#drugRefOv) #globalUtilities{display:none}'));
add('Drug Reference removes redundant helper copy and persistent catalog count',!demo.includes('Search 100 concise pharmacist monographs')&&!demo.includes('Concise educational reference. Verify the current label')&&demo.includes('id="drugRefCount" role="status" aria-live="polite"></span>'));
add('Study Cards action is compact, reversible, and follows the medication source',demo.includes("drugMonoHTML(m.key,true,action)")&&demo.includes("drug-ref-save-row")&&demo.includes("toggleDrugFromReference")&&demo.includes("Saved to Study Cards")&&demo.includes("Remove</button>"));
add('Drug Reference reuses the complete monograph and Study Cards flow',demo.includes("drugReferenceDetailHTML(key)")&&demo.includes("drugMonoHTML(m.key,true,action)")&&demo.includes('function addDrugFromReference')&&demo.includes('Saved to Study Cards'));
...
add('Drug Reference hides empty letters and enlarges mobile targets',demo.includes("filter(function(letter){return drugReferenceKeys('',letter).length>0;})")&&demo.includes('.drug-ref-letter{width:28px;height:32px}'));
add('Drug Reference supports modal focus and result keyboard navigation',demo.includes('function trapModalFocus')&&demo.includes("['ArrowDown','ArrowUp','Home','End']")&&demo.includes('role="status" aria-live="polite"'));
