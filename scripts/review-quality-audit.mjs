...
(()=>{
 ...
 for(const x of cases){
  ...
  const learnStart=out.indexOf('<div class="review-panel" data-rpanel="learn"'),reviewOnly=learnStart>=0?out.slice(0,learnStart):out;
  const expectedSources=reviewSourceEntries(c),hasSources=/<h4>U\.S\. sources?<\/h4>/.test(reviewOnly)&&/<a href=/.test(reviewOnly);if(expectedSources.length)strictSourceExpected++;if(Boolean(expectedSources.length)===hasSources)strictSourceMatched++;if(!expectedSources.length&&!/review-sources/.test(reviewOnly))noFallbackSource++;if(!expectedSources.length||expectedSources.every(r=>out.includes('Used for '+String(r.support).toLowerCase())))sourceSupportNamed++;if(caseReferences(c).every(isUSReference))usSourceCases++;
